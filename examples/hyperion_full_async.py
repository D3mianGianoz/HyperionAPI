...
class AsyncHyperion(object):
    ...
    async def get_channel_count(self):
        ...
        :type: int
        """
        return unpack("I", await self._execute_command("#GetDutChannelCount").content)[
            0
        ]
    ...
    async def get_available_detection_settings(self):
        ...
        detection_settings_data = await self._execute_command(
    ...
    async def get_channel_detection_setting(self, channel: int):
        ...
        :rtype: HPeakDetectionSettings
        """
        id_data = await self._execute_command(
    ...
    async def set_peak_offsets_in_wavelength(
        ...
        delays = delays or np.asarray(
            np.round(
                2
                * (
                    np.array(distances, dtype=np.float)
                    * index_of_refraction
                    / SPEED_OF_LIGHT
                    * 1e9
                )
